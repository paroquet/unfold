...
## 0. 已裁决：verify 失败 = 拒绝出货（2026-09-16）

**S1 已由用户裁决：暂时拒绝出货，不降级。** spec §3 已改写为与 §6.3 一致，
Plan 1 的实现（`run.ts` 直接抛 `VerifyError`）无需改动。

「暂时」是字面意思：将来若要设计降级路径（失败时保留 `SNAP` 上的 worktree 供人读
整体改动），应作为独立的设计决定重开。

**连带解锁**：第 3 节的「错误路径残留 ref 与 worktree」此前卡在这条矛盾上——
既然不降级，失败路径上的产物就没有保留价值，**应当清理**，实现留给 Plan 2/3。

<details>
<summary>原矛盾记录（存档）</summary>
