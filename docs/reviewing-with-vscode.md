...
- `order`：路径前缀数组，命中的整体排到最前、按给定顺序；桶内仍然走依赖序。
  ...
  非源码文件（`.md` / `.json` / 锁文件 / 图片 / 无扩展名的仓库元数据）不参与依赖图，
  **一律排在最后**、按路径字典序——`order` 也提不动它们，免得整条叙事从一篇 README 讲起。
...
改了 `maxFiles` / `order` / `pair` 会让**规则指纹**变化。指纹变化**不会**自动
重排章节——册照样沿用，只是会打印一行提示：

```
规则已变更（指纹 a1b2c3 → d4e5f6）。要按新规则重排章节请跑 --reset-chapters。
```

想真的按新规则重新分章，得自己跑 `--reset-chapters`（见下一节）。
只改 `titles` 指纹不变——纯文案不该让你已有的批注失效。
...
```
依赖证据
  扫描   30 个单元（TS 30）｜跳过 27 个（未支持依赖扫描的文件类型 24：.md 13、.json 5、.yaml 2、其他 4；未配对到实现 3：.ts 3）
  单元   本轮 83 个文件 → 57 个单元（实现与配对到它的测试算一个）
  连边   92 条跨文件依赖
  破环   无强连通分量
  建议   "order": ["packages/core/src/bin", "packages/core/src/git", "packages/core/src/narrate"]
```

数的单位是**单元**不是文件：一个单元 = 一个实现 + 配对到它的测试，所以
`扫描 + 跳过` 得到的是单元数，`单元` 那行再把「你改了 83 个文件 → 配对成
57 个单元」这笔账摊开，让你能跟自己这次的改动对上。跳过的原因分两类：
`未支持依赖扫描的文件类型` 是文档/配置/图片这类本来就没有 import 可扫的，
`未配对到实现` 是某个测试文件规约不到任何实现、canonical 路径是配对**虚构**
出来的（仓库里并不存在那个文件）。

这一块只给证据，不下结论：扫了多少、跳过了哪些（按语言与扩展名诚实标出原因，
